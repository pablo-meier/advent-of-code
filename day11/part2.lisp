...
;;
;; Note that we have to do MAD TREE PRUNING for this to run anywhere near a reasonable amount of
;; time. That's mostly in the following:
;;
;; * Never consider a state you've already considered (i.e. move something up, then move it down).
;; * States are isomorphic on pairs! Polonium and Ruthenium chip/generators swapped mean the same moves.
;; * Never go down states when a floor is already empty/conquered.
;; * Never move one thing up when you can move two things up.

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Lazy man's Queue for the BFS, per that famous interview question of
;; "Can you make an (amortized) Constant-Time Queue from two stacks?"
...
(defun microchipp (sym)
  (member sym '(ELERIUM-MICROCHIP DILITHIUM-MICROCHIP POLONIUM-MICROCHIP THULIUM-MICROCHIP PROMETHIUM-MICROCHIP RUTHENIUM-MICROCHIP COBALT-MICROCHIP)))
(defun generatorp (sym)
  (member sym '(ELERIUM-GENERATOR DILITHIUM-GENERATOR POLONIUM-GENERATOR THULIUM-GENERATOR PROMETHIUM-GENERATOR RUTHENIUM-GENERATOR COBALT-GENERATOR)))
    ...
    (if (eq sym 'THULIUM-MICROCHIP) 'THULIUM-GENERATOR
      (if (eq sym 'PROMETHIUM-MICROCHIP) 'PROMETHIUM-GENERATOR
        (if (eq sym 'RUTHENIUM-MICROCHIP) 'RUTHENIUM-GENERATOR
          (if (eq sym 'DILITHIUM-MICROCHIP) 'DILITHIUM-GENERATOR
            (if (eq sym 'ELERIUM-MICROCHIP) 'ELERIUM-GENERATOR
              'COBALT-GENERATOR)))))))
    ...
    (if (eq sym 'THULIUM-GENERATOR) 'THULIUM-MICROCHIP
      (if (eq sym 'PROMETHIUM-GENERATOR) 'PROMETHIUM-MICROCHIP
        (if (eq sym 'RUTHENIUM-GENERATOR) 'RUTHENIUM-MICROCHIP
          (if (eq sym 'DILITHIUM-GENERATOR) 'DILITHIUM-MICROCHIP
            (if (eq sym 'ELERIUM-GENERATOR) 'ELERIUM-MICROCHIP
              'COBALT-MICROCHIP)))))))
...
(defun set-visited (state)
  (let* ((naked (naked-state state)))
...
(defun traverse ()
  ...
  (let* ((curr-state (dequeue)))
    (if (seen-before curr-state)
      (traverse)
      (let* ((_ (set-visited curr-state))
